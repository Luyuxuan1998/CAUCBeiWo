...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	$firststr='<span class="jumppage total-page">共<span id="max-page">'.$num.'</span>条</span>';
	//上一页
	if($page<>1)
	{
		...
		$prepage='<a class="prev page-numbers" href="'.$dolink.$prido.'"><i class="fa fa-chevron-left"></i></a>&nbsp;&nbsp;';
	}
	//下一页
	if($page!=$totalpage)
	{
		...
		$nextpage='<a class="next page-numbers" href="'.$nextpagelink.'"><i class="fa fa-chevron-right"></i></a>&nbsp;&nbsp;';
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1="<span class='page-numbers current'>";
			$is_2="</span>&nbsp;";
		}
		elseif($i==1)
		{
			$is_1='<a class="page-numbers" href="'.$dolink.$add['dofile'].$type.'">';
			$is_2="</a>&nbsp;";
		}
		else
		{
			$thispagelink=$repagenum&&$repagenum<$i?eReturnRewritePageLink2($add,$i):$dolink.$add['dofile'].'_'.$i.$type;
			$is_1='<a class="page-numbers" href="'.$thispagelink.'">';
			$is_2="</a>&nbsp;";
		}
		$returnstr.=$is_1.$i.$is_2;
	}
	$returnstr=$prepage.$returnstr.$nextpage.$firststr;
...
function sys_ShowTextPage($totalpage,$page,$dolink,$add,$type,$search=""){
	...
	$snum=2;
	//$totalpage=ceil($num/$line);//取得总页数
	$firststr='<span class="jumppage total-page">共<span id="max-page">'.$totalpage.'</span>页</span>';
	//上一页
	if($page<>1)
	{
	...
	{
		$pagenex=$page+1;
		$nextpage='<a class="next page-numbers" href="'.$dolink.$add[filename].'_'.$pagenex.$type.'"><i class="fa fa-chevron-right"></i></a>&nbsp;&nbsp;';
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		elseif($i==1)
		{
			$is_1='<a class="page-numbers" href="'.$dolink.$add[filename].$type.'">';
			$is_2="</a>&nbsp;";
		}
		else
		{
			$is_1='<a class="page-numbers" href="'.$dolink.$add[filename].'_'.$i.$type.'">';
			$is_2="</a>&nbsp;";
		}
		$returnstr.='&nbsp;'.$is_1.$i.$is_2;
	}
